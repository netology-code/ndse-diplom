...
Модуль объявлений используется функциональными модулями для показа списка объявлений и для размещения и удаления
объявлений.
...
Результатом работы функции должен быть `Promise`, который объектом модели `Chat` или `null`
...
По `id` собеседника и `id` текущего пользователя нужно найти чат через функицю "Получить чат между пользователями". Далее для этого чата нужно получить историю сообщений и отправить ее в ответ c событием `chatHistory`
...
При подключении нового клиента должна создаваться подписка на новые сообщения в чате (модуль чат). Полученно сообщение передаётся целиком клиенту.
